...
fun <T> bind(r: Option<T>): Option<T> {
    return if (r is Some) {
        ...
        // Works incorrectly (see previous line) and does not diagnose an error (see KT-10896)
        <!TYPE_INFERENCE_FAILED_ON_SPECIAL_CONSTRUCT!>if<!> (true) <!TYPE_INFERENCE_EXPECTED_TYPE_MISMATCH!>None()<!> else <!DEBUG_INFO_SMARTCAST!>r<!>
    }
    else r
}
